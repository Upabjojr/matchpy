...
from sympy_objects.operations import SIN, EQUALITY
...
class TestManyToOneMatcherWithFreeQ:
    ...
    def test_extract_coefficients(self):
        ...
        _, subst = results[0]
        assert matchpy_to_sympy(subst['a']) == y
        assert matchpy_to_sympy(subst['b']) == 3 * z
        assert matchpy_to_sympy(subst['c']) == 5
