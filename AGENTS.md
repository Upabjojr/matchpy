...
that ports the Rubi rule set to Python on top of the core matcher (now renamed
**OmniMatch**) + SymPy.

The `omnimatch/` package is a **highly customized, strict fork of MatchPy**,
deliberately stripped of the runtime magic, dynamic class monkey-patching, and
forced-inheritance dependencies of the official version. The work is split across four Python
packages, layered bottom-up:

| Package           | May import          | Purpose |
|-------------------|---------------------|---------|
| `omnimatch/`        | *(stdlib + `multiset` only)* | Core pattern matching — expression trees, wildcards, one-to-one and many-to-one matching, discrimination nets. Value classes subclass the lightweight `omnimatch._typed.TypedModel` (see [Conventions](#conventions)). |
| `sympy_matching/` | `omnimatch`           | Bridge between SymPy and OmniMatch. Converts SymPy trees ↔ OmniMatch trees (singledispatch), maps SymPy heads (`Add`, `Mul`, `Pow`, `sin`, …) to OmniMatch `OperationHead`s, and adds `WildSymbol` (a SymPy symbol that becomes a OmniMatch `Wildcard`). |
| `sympy_wolfram/`  | `omnimatch`, `sympy_matching` | Data structures and behaviors for standard Wolfram Mathematica objects, plus a converter from Mathematica **Full-Form List (FFL)** ASTs (JSON) into SymPy code. Hosts the `MathematicaExpr` base class. |
...
- **Decoupled translation layer** — external expression trees are converted into
  OmniMatch's native tree structure by explicit translator functions *before*
...
boundary: it recursively transforms SymPy AST nodes into native OmniMatch trees
...
**Layer 1 — Core matching engine (`omnimatch/`).** The foundational matcher,
...
SymPy and OmniMatch. Imports only from `omnimatch/`; has no visibility into
...
|------------------|------------------------------------------|---------------------------------------------|----------------------|
| `omnimatch`        | Standard Python only                     | `sympy_matching`, `sympy_wolfram`, `rubi_rules` | Purely stateless data matching |
| `sympy_matching` | `omnimatch`                                | `sympy_wolfram`, `rubi_rules`               | Functional tree transformation |
| `sympy_wolfram`  | `omnimatch`, `sympy_matching`              | `rubi_rules`                                | Definitions of standard Wolfram AST schemas |
| `rubi_rules`     | `omnimatch`, `sympy_matching`, `sympy_wolfram` | *(none)*                                | Evaluation logic, utility extensions, rule execution |
...
Rubi .m rules → (precomputed) FFL JSON → codegen/generate.py
    ...
    → build_tracing_replacer() → OmniMatch ManyToOneReplacer
...
- `omnimatch/expressions/` — `expressions.py` (`TypedModel` expression types), `constraints.py`, `substitution.py`, `functions.py`; `omnimatch/_typed.py` — the `TypedModel` base
- `omnimatch/matching/` — `one_to_one.py`, `many_to_one.py`, `syntactic.py` (discrimination net), `bipartite.py`, `hopcroft_karp.py`, `code_generation.py`, `json_serialization.py`
- `sympy_matching/` — `operations.py` (the `SYMPY_NODES` head table), `conversion.py` (singledispatch converters), `wild.py`, `registered_heads.py`, `json_ext.py`, `constraint.py` (`SymPyMatchingConstraint`, the generic SymPy-side constraint base), `matching_rule.py` (`SymPyReplacementPattern`, `build_tracing_replacer`, `_make_omnimatch_constraint`, `_make_replacement_fn` — the generic rule/replacer machinery, no Wolfram/Rubi dependency)
...
- `tests/` — OmniMatch core tests; `<package>/tests/` — per-package tests

## Environment & running

- **Python 3.10 or newer** (`setup.cfg` sets `python_requires = >=3.10`; the newer packages use 3.10+ syntax like `str | None`).
- Core runtime deps: `multiset` (omnimatch), plus `sympy` and `pydantic` for the layers above `omnimatch/` (`sympy_matching.matching_rule.SymPyReplacementPattern` is a pydantic `BaseModel`; only `omnimatch/` itself is pydantic-free). Install dev extras with `make init` (`pip install .[develop]`).
- **OmniMatch core tests + doctests:** `make test` (`py.test tests/ --doctest-modules omnimatch/ README.rst docs/example.rst`)
...
- OmniMatch value classes subclass **`omnimatch._typed.TypedModel`** (Pydantic was removed for speed). Declare fields via class annotations; use `field(default_factory=...)` (from `omnimatch._typed`) for mutable defaults (list/dict/set). `TypedModel.__init__(**kwargs)` assigns fields, applies defaults, and enforces a **shallow** `isinstance` type-check per field at construction (outer type only — `List[X]` is checked as `list`). Private attrs (`_name`) aren't fields; a subclass may turn an inherited field into a bare `ClassVar` (fixed class-level value) and it stops being an instance field. Don't reintroduce a Pydantic dependency **inside `omnimatch/`** — the ban is scoped to the core package; higher layers still use pydantic (e.g. `SymPyReplacementPattern`).
- Prefer the existing **singledispatch** converters in `sympy_matching/conversion.py` when adding SymPy↔OmniMatch support; register new heads through `register_sympy_head` / the `SYMPY_NODES` table in `sympy_matching/operations.py`.
